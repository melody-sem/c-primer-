 ...
 {  
     ...
     while ((ch = getchar()) != 'q' && ch != 'Q')  {  
        ...
        else if (lb_total < 5.0)  shipping = under5;  
        else if (lb_total < 20)  shipping = under20;  
        else  shipping = base20 + extralb * lb_total;  

        if (cost_total > 100.0)  discount = DISCOUNT_RATE * cost_total; 
        else  discount = 0.0;  
        ...
        if (cost_total > 100)  printf("Volume discount: $%.2f\n", discount);  
        printf("Shipping: $%.2f\n", shipping);  
